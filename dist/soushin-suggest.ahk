...
; ランチャーの掴みしろ(LauncherDragBar)ドラッグは、GetKeyState("LButton","P")ポーリング
; (LauncherWatchDrag、30ms→5ms→当たり判定+6pxと段階的に手直ししてきたが、タッチパッドでの
; 「掴もうとすると逃げる/ずれる」報告が解消しなかった。世界的に確立された代替策(AutoHotkey
; コミュニティの定番)である「WM_NCLBUTTONDOWN(タイトルバーを掴んだのと同じメッセージ)を
; OSへ直接投げる」方式に置き換える(2026-07-23)。OS自身のネイティブなウィンドウ移動処理に
; 完全委譲するため、ポーリング間隔やタッチパッドのAAP/ジェスチャー再分類の影響を受けない。
; 過去に試して「非クライアント処理に委譲できない」として撤回した記録があるが、あれはおそらく
; PostMessageの宛先をコントロール(LauncherDragBarのhwnd)にしてしまっていたため。
; 非クライアント領域を持つのは最上位ウィンドウだけなので、宛先は必ず親Gui(LauncherGui.Hwnd)
; にする必要がある。OnMessageはスクリプト全体で共有されるため、hwnd引数で対象コントロールを
...
; ランチャーの表示位置。"cursor"=カーソル近く(画面端は反転/クランプ、既定)、"center"=カーソルのある
; モニタの中央固定(旧既定・v1.13.0〜v1.23.2相当)。どちらが快適かは環境(タッチパッド/マウス、
; モニタ構成)次第という実機フィードバックを受け、設定で選べるようにした(2026-07-23)。
global LauncherPositionMode := "cursor"
...
ShowLauncher() {
    ...
    ; 表示位置は設定(launcher.position)でカーソル追従/中央固定を選べる(2026-07-23、設定画面参照)。
    ; Cliborは「キャレット位置」か「マウスカーソル位置」を選べる。キャレット位置はAutoHotkeyの
    ; A_CaretX/YがChrome系UI・Electron製アプリ等で取得失敗(0/空)しやすい既知の弱点があるため
    ; 採用せず、どのアプリでも確実なマウスカーソル位置を使う。過去(v1.13.0)にカーソル追従を
    ; 「画面下端で隠れる」として撤回した記録があるが、これは追従自体でなく「はみ出し対策が
    ; 無かった」ことが原因。右クリックメニュー等のOS標準UIと同じ「はみ出す分だけ画面内へ
    ; クランプ」を追加すれば両立できる。マルチモニタはMonitorRectAtCursor()(範囲指定スクショ
    ; 機能と共用の既存ヘルパー)でカーソルのあるモニタ範囲を取得。
    ; GetPosはShow()より前だと高さ0を返す(未作成ウィンドウのため)。「画面外の座標に実際に
    ...
    if (LauncherPositionMode = "center") {
        if mr {
            launcherX := mr.l + (mr.w - launcherW) // 2
            launcherY := mr.t + (mr.h - launcherH0) // 2
        } else {
            launcherX := (A_ScreenWidth - launcherW) // 2
            launcherY := (A_ScreenHeight - launcherH0) // 2
        }
    } else {
        MouseGetPos(&curX, &curY)
        ; カーソル直下だとクリック点を覆うので少しずらす(右クリックメニューと同じ流儀)。
        ; 単純な画面端クランプだと、カーソルが下端/右端に近いとき毎回そのまま縁に張り付いて
        ; しまい「いつも隅に出る」ように見える(2026-07-23実機報告)。右クリックメニューと同じく
        ; 「下右に置く余地が無ければ上/左へ反転」してから、最後の保険として端クランプもかける。
        if mr {
            launcherX := (curX + 12 + launcherW <= mr.r) ? curX + 12 : curX - 12 - launcherW
            launcherY := (curY + 12 + launcherH0 <= mr.b) ? curY + 12 : curY - 12 - launcherH0
            if (launcherX + launcherW > mr.r)
                launcherX := mr.r - launcherW
            if (launcherX < mr.l)
                launcherX := mr.l
            if (launcherY + launcherH0 > mr.b)
                launcherY := mr.b - launcherH0
            if (launcherY < mr.t)
                launcherY := mr.t
        } else {
...
; 名前付き関数+global宣言で書く(fat arrowは握りつぶされる既知の地雷。上のコメント参照)。
ApplyLauncherPositionSetting(v) {
    global LauncherPositionMode
    LauncherPositionMode := (v = "center") ? "center" : "cursor"
}
...
SettingDefsInit() {
    ...
    SettingDefs := Map(
        ...
        "launcher.position", {section: "launcher", default: "cursor", apply: ApplyLauncherPositionSetting},
...
global SettingsRadioPosCursor := 0, SettingsRadioPosCenter := 0
...
ShowSettingsWindow(*) {
    ...
    ; ランチャー表示位置(2026-07-23)。環境(タッチパッド/マウス、モニタ構成)によってカーソル追従と
    ; 中央固定のどちらが快適か分かれるという実機フィードバックを受け、選べるようにした。
    SettingsGui.Add("GroupBox", "x10 y316 w380 h64", "ランチャーの表示位置")
    SettingsRadioPosCursor := SettingsGui.Add("Radio", "x20 y338 w360", "マウスカーソルの近くに表示する（既定）")
    SettingsRadioPosCenter := SettingsGui.Add("Radio", "x20 y360 w360", "画面中央に固定表示する")
    SettingsRadioPosCursor.Value := (LauncherPositionMode != "center")
    SettingsRadioPosCenter.Value := (LauncherPositionMode = "center")
    SettingsRadioPosCursor.OnEvent("Click", (*) => SetSetting("launcher.position", "cursor"))
    SettingsRadioPosCenter.OnEvent("Click", (*) => SetSetting("launcher.position", "center"))

    SettingsGui.Add("Button", "x20 y388 w150 h28", "保存フォルダを開く").OnEvent("Click", OpenArchiveDir)
    SettingsGui.Add("Button", "x290 y388 w100 h28", "閉じる").OnEvent("Click", (*) => HideSettingsWindow())
